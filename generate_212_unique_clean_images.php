...
// Global set to ensure NO DUPLICATE PHOTO IDS ACROSS ALL 212 IMAGES
$usedPhotoIds = [];
...
// Clean old media folders to eliminate duplicate structures
echo "Cleaning existing media directories for a clean, non-duplicate hierarchy...\n";
...
foreach ($articlesSpec as $id => $spec) {
    ...
    // Process 4 unique images per article
    for ($i = 1; $i <= 4; $i++) {
        $query = $queries[$i - 1];
    ...
    echo "✓ Processed Article #{$id} [{$shortSlug}]: 4 unique images generated.\n";
}
