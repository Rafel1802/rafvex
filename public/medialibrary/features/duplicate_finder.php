...
function duplicate_finder_handle(array $post, string $baseDir, string $currentDir, string $baseUrl, string $current): void {
    ...
    if (!$byHash && !$byName && !$bySize) $byHash = true; // default
...
function duplicate_collect_files(string $dir, string $relBase): array {
    ...
    foreach ($it as $file) {
        ...
        if (str_starts_with($file->getPath(), $dir . '/.versions')) continue;
        if (str_starts_with($file->getPath(), $dir . '/.trash')) continue;
...
function duplicate_group(array $files, bool $byHash, bool $byName, bool $bySize, string $baseUrl, string $baseDir): array {
    ...
    foreach ($files as $f) {
        $keys = [];
        if ($byHash && $f['sha256']) $keys[] = 'hash:' . $f['sha256'];
        if ($byName)                 $keys[] = 'name:' . strtolower($f['name']);
        if ($bySize)                 $keys[] = 'size:' . $f['size'];
    ...
    foreach ($buckets as $key => $group) {
        ...
        if (isset($seen[$pathSet])) continue;
        $seen[$pathSet] = true;
