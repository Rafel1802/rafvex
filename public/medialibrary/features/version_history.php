...
function version_history_handle(string $action, array $post, string $baseDir, string $currentDir, string $current, string $userId, callable $cleanExistingName, callable $formatSize, callable $autoBackup): void
{
    ...
    if ($action === 'restore_version') {
        ...
        if (! $versionId) {
            echo json_encode(['success' => false, 'error' => 'Invalid version ID']);
            exit;
        }
        ...
        if (! is_file($storedAbs)) {
            echo json_encode(['success' => false, 'error' => 'Version file missing on disk']);
            exit;
        }
        ...
        if (! @copy($storedAbs, $originalAbs)) {
            echo json_encode(['success' => false, 'error' => 'Could not restore version file']);
            exit;
        }
    ...
    if ($action === 'download_version') {
        ...
        $storedAbs = AI_VERSIONS_DIR.'/'.$sp;
        if (! is_file($storedAbs)) {
            echo json_encode(['success' => false, 'error' => 'Version file missing']);
            exit;
        }
