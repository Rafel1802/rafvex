...
export default function Create({ auth }: { auth: any }) {
  ...
  return (
    <AdminLayout auth={auth}>
      <Head title="Create Pop up ad — Rafvex CMS" />
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">
                Setup a new sponsored customer popup ad with media, button link, and scheduling.
              </p>
            </div>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-7 space-y-6">
            ...
            <div className="bg-white dark:bg-slate-900 p-5 sm:p-6 rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-2xs space-y-4">
              ...
              <div>
                <label className="block text-xs font-bold uppercase tracking-wider text-slate-600 dark:text-slate-300 mb-1.5">
                  Upload Media File (Image, GIF, or Video up to 50MB)
                </label>
                <div className="border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-2xl p-6 text-center hover:border-red-400 dark:hover:border-red-500 transition cursor-pointer bg-slate-50/50 dark:bg-slate-800/40">
                  <input
                    type="file"
                    id="media-upload"
                  ...
                  <label htmlFor="media-upload" className="cursor-pointer flex flex-col items-center">
                    <div className="w-12 h-12 rounded-2xl bg-red-50 dark:bg-red-950/50 text-red-600 flex items-center justify-center mb-2">
                      <Upload size={20} />
                    </div>
                    ...
                    {data.media_file && (
                      <span className="mt-2 text-xs font-bold text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/60 px-3 py-1 rounded-full">
                        Selected: {data.media_file.name} ({(data.media_file.size / (1024 * 1024)).toFixed(2)} MB)
                      </span>
                    )}
                  </label>
                </div>
              ...
              <div>
                <label className="block text-xs font-bold uppercase tracking-wider text-slate-600 dark:text-slate-300 mb-1.5">
                  Or Direct Media / Video Stream URL
                </label>
                <input
                  type="url"
            ...
            <div className="flex items-center gap-3">
              ...
              >
                {processing ? 'Creating Ad...' : 'Save & Launch Pop up ad'}
              </button>
